...
DEFAULTS = {
    ...
    'fig_width': 25,
    'fig_height': 14,
    'logo_x': 1800,  # Adjusted default X position for top right (increased range)
    'logo_y': 1100,  # Adjusted default Y position for top right (increased range)
    'logo_size': 250,  # Increased default max size
...
def generate_stacking_plan_plot(data, building_name, start_color, end_color,
    ...
    # Ensure that 'Floor' and 'Suite Number' columns are of numeric type for sorting
    data['Floor'] = pd.to_numeric(data['Floor'], errors='coerce')
    data['Suite Number'] = pd.to_numeric(data['Suite Number'], errors='coerce')

    # Drop rows where 'Floor' or 'Suite Number' became NaN due to coercion errors
    data.dropna(subset=['Floor', 'Suite Number'], inplace=True)
    ...
    for floor in floors:
        ...
        ax.text(-0.5, y_pos, f"Floor {int(floor)}\n{floor_sum:,} SF", # Ensure floor is int for display
        ...
        for i, row in floor_data.iterrows():
            ...
            ax.text(x=x_pos + width/2, y=y_pos - 0.2, s=f"Suite {int(suite)}", # Ensure suite is int for display
                    ha='center', va='center', fontsize=6)
    ...
    if logo_file_to_display is not None:
        ...
        # Convert figure dimensions from inches to pixels (assuming 100 DPI for calculation)
        # This helps in positioning the logo more intuitively relative to the chart's pixel dimensions.
        fig_width_pixels = fig_width * fig.dpi
        fig_height_pixels = fig_height * fig.dpi

        # Adjust logo_x, logo_y to be relative to the top-right corner,
        # but the figimage function expects from bottom-left.
        # So, we calculate bottom-left pixel coordinates for top-right placement.
        # `logo_x` is distance from left edge, `logo_y` is distance from bottom edge.
        # To place in top-right:
        # X: fig_width_pixels - logo_size - margin (logo_x acts as right margin here)
        # Y: fig_height_pixels - logo_size - margin (logo_y acts as top margin here)
        
        # If logo_x and logo_y are meant as *margins* from the top/right, adjust calculation:
        # For top-right: xo = fig_width_pixels - logo_size - logo_x_margin
        #                yo = fig_height_pixels - logo_size - logo_y_margin
        # Given the slider names `logo_x` (from left) and `logo_y` (from bottom),
        # we'll map them to direct pixel coordinates.
        # Let's assume `logo_x` is the pixel distance from left, `logo_y` is pixel distance from bottom.
        # If we want top-right, the user needs to set `logo_x` to a high value and `logo_y` to a high value.
        # The current defaults for logo_x=1800, logo_y=1100 correspond to specific pixel values.
        
        # For initial top-right, the default `logo_x` and `logo_y` (1800, 1100) are used directly as pixel coordinates.
        logo.thumbnail((int(logo_size), int(logo_size))) # Resize logo
...
with st.sidebar:
    ...
    if logo_file_to_display is not None:
        # Increased max values for more flexibility
        logo_x = st.slider(
            "Logo X position (pixels from left)", 0, 3000, value=st.session_state.logo_x, step=10, key="logo_x_slider"
        )
        ...
        logo_size = st.slider(
            "Logo max size (pixels)", 50, 800, value=st.session_state.logo_size, step=10, key="logo_size_slider"
        )
    ...
    with st.spinner("📊 Generating stacking plan..."):
        try:
            ...
            if missing_cols:
                ...
                st.stop()
            # Ensure Floor and Suite Number are numeric before sorting
            for col in ['Square Footage', 'Floor', 'Suite Number']: # Added Suite Number here
